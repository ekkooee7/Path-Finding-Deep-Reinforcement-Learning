...
class Environment():
    ...
    def reset_env_random(self, random=True):
        '''
        重设agent的位置
        :return:
        '''
...
class Agent():
    def __init__(self, location, map_observation, agents_observation, gamma=0.99, epsilon=0.9, epsilon_min=0.01,
                 epsilon_decay=0.9, target_update=16):
        ...
        self.ac_model = AC_CNN_MLP().to(device)
        self.ac_optimizer = optim.Adam(self.ac_model.parameters(), lr=0.001)

        # self.critic = Critic_CNN_MLP().to(device)
        # self.actor = Actor_CNN_MLP().to(device)
        #
    ...
    def update(self, fov, h, action, reward, fov_next, h_next, done):
        ...
        td_target = reward + self.gamma * self.ac_model(fov_next, h_next, policy_output=False) * (1 - done)
        td_delta = td_target - self.ac_model(fov, h, policy_output=False)  # 时序差分误差
        log_probs = torch.log(self.ac_model(fov, h, policy_output=True).gather(1, action))
        actor_loss = torch.mean(-log_probs * td_delta.detach())
        # 均方误差损失函数
        critic_loss = torch.mean(F.mse_loss(self.ac_model(fov, h, policy_output=False), td_target.detach()))
    ...
    def act(self, fov, h):
        ...
        act_values = self.ac_model(fov, h, policy_output=True)
        self.action = np.argmax(act_values.cpu().detach().numpy())
...
#
# class Actor_CNN_MLP(nn.Module):
#     def __init__(self, additional_vector_length=9):
#         super(Actor_CNN_MLP, self).__init__()
#         # 卷积层
...
#
#
# class Critic_CNN_MLP(nn.Module):
#     def __init__(self, additional_vector_length=9):
#         super(Critic_CNN_MLP, self).__init__()
#         # 卷积层
#         self.conv1 = nn.Conv2d(in_channels=1, out_channels=4, kernel_size=3, stride=1, padding=1)
#         self.conv2 = nn.Conv2d(in_channels=4, out_channels=8, kernel_size=3, stride=1, padding=0)
#         # 全连接层的输入需要加上额外向量的长度
...
class AC_CNN_MLP(nn.Module):
    def __init__(self, additional_vector_length=9):
        super(AC_CNN_MLP, self).__init__()
        # 卷积层
        ...
        # 全连接层的输入需要加上额外向量的长度
        self.fc1_input_size = 32 * 2 * 2 + additional_vector_length
        self.fc1 = nn.Linear(self.fc1_input_size, 512)
        self.fc_critic = nn.Linear(512, 1)
        self.fc_actor = nn.Linear(512, 5)

    def forward(self, x, additional_vector, policy_output=True):
        # 应用卷积层和池化层
        x = F.relu(self.conv1(x))
        # 应用第二个卷积层
        x = F.relu(self.conv2(x))
...
def save_model(agent, episode, save_dir='./train_CNN_MLP/CNN_MLP_train'):
    ...
    model_path = os.path.join(save_dir, f'ac_model_episode_{episode}.pt')
    # 保存模型
    torch.save(agent.ac_model, model_path)
...
def train_dqn(episode_count, batch_size, max_step, continue_train=False, model_file=""):
    ...
    save_dir = get_next_available_dir('./train_MLP_A2C/MLP_A2C_train')
    ...
    for episode in range(episode_count):
        ...
        while not done and t <= max_step:
            ...
            # 获得fov_next
            agents_position = env.agents_position[0]
            fov_next = env.map[agents_position[0] - 2:agents_position[0] + 3,
                       agents_position[1] - 2:agents_position[1] + 3]
            ...
            env.agent_1.update(fov_tensor, h, action, reward, fov_next_tensor, h_next, done)
...
if __name__ == '__main__':
    train_dqn(50000, 32, max_step=70)
    # eval_dqn('train_MLP_A2C/MLP_A2C_train_4/critic_model_episode_49000.pt')
